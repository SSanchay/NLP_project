...
tfid = load('../review_files/tfidf.joblib')
# Загружаем модель для sentiment_ML
model_ml = load('../review_files/logreg.joblib')
...
def tokenize_review(review: str):
    ...
    lemmatized_review = ' '.join([wn_lemmatizer.lemmatize(word, tag[0].lower())  # лемматизация с учётом части речи
                                  ...
                                  for word, tag in
                                  nltk.pos_tag(cleaned_review.split())])  # pos_tag - определяет часть речи

    tokenized_review = reg_tokenizer.tokenize_sents([lemmatized_review])  # ревью состоит из токенов
    clean_tokenized_review = ' '.join(
        [word for word in tokenized_review[0] if word not in sw])  # удаляем стоп-слова, сверху объявили sw
...
with open('../review_files/dict.json', 'r') as fp:
...
model_rnn.load_state_dict(torch.load('../review_files/params_gru.pt', map_location=torch.device('cpu')))
model_rnn.eval()
...
model_bert_ml = load('../review_files/log_reg_2.joblib')
...
def sentiment_ML(review: str):
    ...
    tfid_representation = tfid.transform(
        [clean_tokenized_review])  # ревью состоит из векторов, сверху загружаем vectorizer
...
def sentiment_BERT_ML(review: str):
    ...
    tokenized = tokenizer_bert.encode(review,
                                      ...
                                      max_length=max_len)  # добавили служебные токены и обрезали по макс длине
